import cv2
import torch
import seaborn as sns
import matplotlib.pyplot as plt

face_cascade = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
eye_cascade = cv2.CascadeClassifier('haarcascade_eye_tree_eyeglasses.xml')
cap = cv2.VideoCapture(0)

# Torch tensor for image processing
def process_image(img):
    # Convert to Torch tensor
    img_tensor = torch.tensor(img)

    # Apply some image processing using Torch (e.g., flip horizontally)
    img_tensor = torch.flip(img_tensor, dims=[1])

    # Convert back to numpy array
    img_processed = img_tensor.numpy()

    return img_processed

while cap.isOpened():
    _, img = cap.read()

    # Process the image using Torch and Tensor
    img_processed = process_image(img)

    gray = cv2.cvtColor(img_processed, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.3, 4)

    for (x, y, w, h) in faces:
        cv2.rectangle(img_processed, (x, y), (x+w, y+h), (0, 255, 0), 3)
        roi_gray = gray[y:y+h, x:x+w]
        roi_color = img_processed[y:y+h, x:x+w]
        eyes = eye_cascade.detectMultiScale(roi_gray)
        for (ex, ey, ew, eh) in eyes:
            cv2.rectangle(roi_color, (ex, ey), (ex+ew, ey+eh), (0, 255, 255), 5)

    # Display the processed image
    cv2.imshow('img', img_processed)

    # Visualize some analysis using Seaborn
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

# Close all windows
cap.release()
cv2.destroyAllWindows()

# Example Seaborn plot
data = [1, 2, 3, 4, 5]
sns.lineplot(x=range(len(data)), y=data)
plt.title("Seaborn Plot")
plt.show()
